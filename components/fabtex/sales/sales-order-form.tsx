...
import { SalesOrderType, PackingType } from '@/app/generated/prisma/client'
...
import { createSalesOrder, updateSalesOrder } from '@/app/actions/fabtex/sales-order'
...
export function SalesOrderForm({
    ...
    // Header State
    const [type, setType] = useState<SalesOrderType>(initialData?.type || 'LOCAL')
    ...
    // Items State
    const [soItems, setSoItems] = useState<SOItemInput[]>(
    ...
    const [currentItem, setCurrentItem] = useState<Partial<SOItemInput>>({
        packingType: 'EVEN',
        pcs: 0,
        unitSize: 0,
        quantity: 0,
    ...
    const onSubmit = (formData: FormData) => {
        ...
        formData.append('items', JSON.stringify(soItems))
        ...
        startTransition(async () => {
            const result = initialData
                ? await updateSalesOrder(initialData.id, {}, formData)
                : await createSalesOrder({}, formData)

            if (result.success) {
                toast.success(initialData ? 'SO Updated' : 'SO Created')
                router.push('/dashboard/fab-tex/sales/sales-order')
            } else {
                toast.error(result.error || 'Failed to save SO')
            }
        })
    }

    const totalAmount = soItems.reduce((sum, item) => sum + item.amount, 0)
        ...
        <form action={onSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 border rounded-lg bg-slate-50 dark:bg-slate-900/50">
                ...
                <div className="space-y-2">
                    ...
                    <Select
                        name="type"
                        value={type}
                        onValueChange={(v) => setType(v as SalesOrderType)}
                    >
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                ...
                <div className="space-y-2">
                    <Label>{type === 'EXPORT' ? 'Ext Ref No' : 'PO No (Cust)'}</Label>
                    <Input
                        name="referenceNo"
                ...
                <div className="space-y-2">
                    <Label>{type === 'EXPORT' ? 'Valid Until' : 'Delivery Date'}</Label>
                    <Input
                        type="date"
                        name="documentDate"
            ...
            <div className="border rounded-xl p-6 bg-card shadow-sm space-y-6">
                    ...
                    <Table>
                        <TableHeader className="bg-slate-50 dark:bg-slate-800">
                            <TableRow>
                                ...
                                <TableHead className="font-bold text-center">{soItems.find(i => i.packingUnitSymbol)?.packingUnitSymbol || 'Pkgs'}</TableHead>
                ...
                <div className="flex justify-end pt-2">
                    <div className="flex gap-6 items-baseline p-4 rounded-xl bg-primary/5 border border-primary/10">
                        <span className="font-medium text-muted-foreground uppercase tracking-widest text-xs">Net Total:</span>
            ...
            <div className="flex justify-end gap-4 p-4 border-t">
                ...
                <Button type="submit" disabled={isPending} size="lg" className="px-10">
                    ...
                    {initialData ? 'Update Sales Order' : 'Confirm & Save Sales Order'}
                </Button>
            </div>
        </form >
    )
