...
import { createSalesReturn, updateSalesReturn } from '@/app/actions/fabtex/sales-return'
...
export function SalesReturnForm({
    ...
    const [returnNumber, setReturnNumber] = useState(initialData?.returnNumber || `SR-${Date.now().toString().slice(-6)}`)
    const [date, setDate] = useState(initialData?.date ? new Date(initialData.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0])
    const [remarks, setRemarks] = useState(initialData?.remarks || '')
    const [selectedInvoice, setSelectedInvoice] = useState<any>(initialData?.salesInvoice || null)
    ...
    const [items, setItems] = useState<any[]>(initialData?.items?.map((item: any) => ({
        ...
        invoicedQty: item.salesInvoiceItem?.invoicedQty || 999999,
    ...
    const onInvoiceChange = (invoiceId: string) => {
        ...
        setSelectedCustomerId(inv.accountId?.toString() || inv.salesOrder?.accountId?.toString() || '')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedInvoice) formData.append('salesInvoiceId', selectedInvoice.id)
        ...
        const result = returnId
            ? await updateSalesReturn(returnId, { success: false }, formData)
            : await createSalesReturn({ success: false }, formData)
        setLoading(false)

        if (result.success) {
            toast.success(result.message)
            router.push('/dashboard/fab-tex/sales/sales-return')
        } else {
            ...
            <Card className="shadow-lg border-none bg-background/50 backdrop-blur-md">
                ...
                <CardContent className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                ...
                                <SelectContent>
                                    ...
                                    {invoices.map(inv => (
                                        <SelectItem key={inv.id} value={inv.id}>
                                            {inv.invoiceNumber} - {inv.account?.name || 'Customer'}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        ...
                        <div className="space-y-2">
                            <Label className="font-semibold">Customer / Account</Label>
                            <Select
